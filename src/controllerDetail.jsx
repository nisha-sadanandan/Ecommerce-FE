...
const ControllerDetail = () => {
  const { addToCart } = useContext(CartContext);
    ...
    const {controllersid} = useParams();
 
  
    useEffect(() => {
      const fetchProduct = async () => {
        try {
          const response = await axios.get(`http://localhost:3000/api/v1/receiver/get-receiverbyid/${controllersid}`); 
...
<div>
    ...
    <div className="row mt-3">
      <div className="col-4">
      <img src={controllers.image}  alt="product" />
      </div>
      <div className="col-6">
      <h2>{controllers.description}</h2>
      <hr/>
      <div>
      ...
      <div>
        ...
        <span className="m-4 fs-3">Rs {controllers.price}</span>
        <div>
...
<hr/>
  ...
  <button className="btn btn-warning rounded" type="button" onClick={() => addToCart(controllers)}>Add to Cart</button>
